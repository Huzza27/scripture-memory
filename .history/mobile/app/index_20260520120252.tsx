...
export default function Index() {
  ...
  const calendarScale = useRef(new Animated.Value((0))).current;

  const toggleDailyMenu = () => {
    var toValue = dailyVersesMenu ? 0 : 500;
    Animated.timing(dailyMenuAnim, {
      toValue,
      duration: 250,
      useNativeDriver: false,
    }).start();
    toValue = dailyVersesMenu ? 1 : 0
    Animated.timing(arrowRotateAnim, {
      toValue,
      duration: 150,
      useNativeDriver: true
    }).start()
    toValue = dailyVersesMenu ? 15 : 17
  ...
  const rotateInterpolate = arrowRotateAnim.interpolate({
    inputRange: [0, 1],
          ...
          ListHeaderComponent={
            (folders.length > 0 || todayVerses.length > 0) ? (
              <View>
                ...
                <View className="rounded-xl overflow-hidden mb-4" style={{ backgroundColor: theme.surface }}>
                  {/* Header */}
                  <TouchableOpacity onPress={toggleDailyMenu}>
                    <View className="flex-row items-center gap-2 px-3" style={{ height: 44 }}>
                    <Animated.View>
                      <Ionicons name="calendar-outline" size={calendarScale} color={theme.accent} />
                    </Animated.View>
                      <Text
                        className="text-xs font-bold uppercase flex-1"
                        style={{ color: theme.accent, letterSpacing: 0.8 }}
                      >
                        Daily Verses · {DAY_NAMES[todayDay]}
                      </Text>
                      {dailyVersesMenu && (
                        <TouchableOpacity
                          className="flex-row items-center gap-1 px-2.5 rounded-md"
                          style={{ backgroundColor: theme.greenSurface, borderWidth: 1, borderColor: theme.green + "44", height: 26 }}
                          onPress={(e) => { e.stopPropagation(); }}
                        >
                          <Ionicons name="play" size={9} color={theme.greenText} />
                          <Text className="text-xs font-semibold" style={{ color: theme.greenText, letterSpacing: 0.3 }}>
                            Practice All
                          </Text>
                        </TouchableOpacity>
                      )}
                      <Animated.View style={{ transform: [{ rotate: rotateInterpolate }] }}>
                        <Ionicons name="chevron-up" size={14} color={theme.textTertiary} />
                      </Animated.View>
                    </View>
                  </TouchableOpacity>
                    ...
                    <View style={{ borderTopWidth: 1, borderTopColor: theme.border }}>
                      {todayVerses.map((v, i) => (
                        <TouchableOpacity
                          ...
                          style={{
                            height: 52,
                            borderBottomWidth: i < todayVerses.length - 1 ? 1 : 0,
                            borderBottomColor: theme.border,
                          }}
                          onPress={() => router.push(`/verse/${v.id}?autostart=1`)}
                        >
                          <Text className="flex-1 text-sm font-semibold" style={{ color: theme.text }}>{v.reference}</Text>
