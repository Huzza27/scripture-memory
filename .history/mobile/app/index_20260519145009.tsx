              ...
              <View>
                ...
                {todayVerses.length > 0 && (
                  ...
                  <TouchableOpacity onPress={toggleDailyMenu}>
                    <View className="flex-row items-center gap-1.5 h-7 rounded-t-xl" style={{backgroundColor: theme.surface, overflow: "hidden"}}>
                  ...
                  <Animated.View style={{ overflow: "hidden", maxHeight: dailyMenuAnim }}>
                    <View style={{backgroundColor: theme.accent, overflow: "hidden"}} className="rounded-b-xl">
                        <View className="mt-1 mb-2">
                      {todayVerses.map(v => (
                        <TouchableOpacity
                        key={v.id}
                        className="flex-row items-center justify-between rounded-xl px-3.5 mb-0.5 mt-1 border"
                        style={{ backgroundColor: theme.accentSurface, borderColor: theme.accent + "22" }}
                        onPress={() => router.push(`/verse/${v.id}?autostart=1`)}
                        >
                          <View className="flex-1 mr-3">
                            <Text className="text-base font-bold" style={{ color: theme.text }}>{v.reference}</Text>
                            {!prefs.hideVerseText && (
